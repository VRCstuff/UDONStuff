    ...
    public class PortableMenu : UdonSharpBehaviour
    {
        ...
        public override void PostLateUpdate()
        {
            if (userIsInVR)
            {
                ...
                transform.SetPositionAndRotation(menuPosition, Quaternion.LookRotation(directionBetweenHands) * Quaternion.Euler(0, 90, 0));
            ...
            else if (Input.GetKey(menuKey))
            {
                ...
                var menuPosition = head.position + transform.TransformDirection(desktopHeadOffset);

                if (Utilities.IsValid(rigidBody))
                    rigidBody.isKinematic = true;

                transform.SetPositionAndRotation(menuPosition, head.rotation);
                transform.localScale = menuScale;
            }
        }
        ...
        public void ShouldHideMenu()
        {
            ...
            var menuPos = transform.position;
